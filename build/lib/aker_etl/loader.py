...
def _load_txn(
    ...
    *,
    ...
    for snap_id, pid in sorted(touched):
        # Same reasoning as core.lease, and for the same reason (module docstring):
        # an upsert cannot remove a summary group or a charge code that vanished
        # from a changed file, and a changed file is the only case that reaches
        # here at all. Section 9's ON CONFLICT clauses stay -- they still guard a
        # single file that prints the same label twice.
        cur.execute(
            "DELETE FROM core.lease WHERE snapshot_id = %s AND property_id = %s", (snap_id, pid)
        )
        cur.execute(
            "DELETE FROM core.rent_roll_summary_group WHERE snapshot_id = %s AND property_id = %s",
            (snap_id, pid),
        )
        cur.execute(
            "DELETE FROM core.charge_summary WHERE snapshot_id = %s AND property_id = %s",
            (snap_id, pid),
        )
