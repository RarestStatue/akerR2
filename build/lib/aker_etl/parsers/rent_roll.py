...
# Mirrors core.resident.resident_id_fmt. Both exist: the database guarantees the
# invariant, the parser keeps a violation from taking the whole load with it.
RESIDENT_ID_RE = re.compile(r"^t[A-Za-z0-9]+$")
...
def parse_rent_roll(path: Path) -> RentRollFile:
    ...
    dropped_block = False        # the open block's header row was rejected
    ...
    for idx, r in enumerate(rows[6:], start=7):   # sheet_row is 1-based
        ...
        if a != "":
            ...
            # `and g != "Total"`: rule 5 has always excluded the block's Total row
            # from the charge lines; a lease header that happens to print Total in
            # column G must not become a charge code named "Total".
            if g and g != "Total":
        ...
        if g and g != "Total":
            if current is None:
                if dropped_block:
                    # The block's header row was already reported. Its charge
                    # lines are collateral, not separate findings.
                    continue
                out.issues.append(
...
def _parse_lease(
    ...
    # The database enforces this too (core.lease.lease_resident_presence), but a
    # CHECK violation inside COPY takes the whole 50-file transaction down with
    # it. Rejecting the row here costs one lease and records why; letting it
    # through costs the entire load. Same trade as BUG.md item 1.
    if lease.occupancy_status in ("occupied", "notice", "future") and not lease.resident_id:
        issues.append(
            ParsedIssue(severity="error", rule="occupied_row_without_resident",
                        sheet_row=sheet_row,
                        detail={"property_code": property_code, "unit_code": unit_code,
    ...
    if resident_id and not RESIDENT_ID_RE.match(resident_id):
        # Was a warning that kept the row, which then failed core.resident's
        # resident_id_fmt CHECK and rolled back the whole run. An error that drops
        # one row is the honest version of the same finding.
        issues.append(
            ParsedIssue(severity="error", rule="resident_id_format", sheet_row=sheet_row,
                        detail={"property_code": property_code, "unit_code": unit_code,
                                "resident_id": resident_id})
        )
        return None
